...
def preprocess(df: pd.DataFrame, test_size: float = 0.2, random_state: int = 42):
    ...
    # 4. KNN Imputation (k=5 neighbors, uses feature correlations to fill NaN)
    imputer = KNNImputer(n_neighbors=5)
    X_imputed = pd.DataFrame(imputer.fit_transform(X), columns=FEATURES)

    # 5. Stratified split (preserves class ratio in both sets)
    ...
    # 6. Scale — fit on train only, transform both (prevents data leakage)
    scaler = RobustScaler()
...
def _remove_outliers(df: pd.DataFrame, factor: float = 3.0) -> pd.DataFrame:
    """
    Remove rows where any feature value is beyond factor * IQR.
    factor=3.0 is conservative — removes only extreme outliers,
